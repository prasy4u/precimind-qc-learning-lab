...
export function ScientificBasisNote({ goto, text }) {
  return (
...
export function SliderField({ label, value, min, max, step, onChange, suffix, id, hint, signed }) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const LJ_KEY_RUNS = new Set([1, 5, 10, 15, 20]);

export function LJChart({ points, mean, sd, unitMode, eventAnnotation, decimals }) {
...
export function DistributionView({ values, mean, target, sd, showBothMarkers }) {
  const W = 640, H = 170;
