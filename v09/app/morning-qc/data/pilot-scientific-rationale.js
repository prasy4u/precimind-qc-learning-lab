...
export const PILOT_SCIENTIFIC_RATIONALE = {
  ...
  'pilot-2-pbrtqc-population-shift': {
    ...
    rootCauseStatus: 'NOT an analytical root cause (rootCauseEstablished=false, corrected during the Stage 12A independent-audit closure). The observed signal IS explained — via signalExplanationEstablished=true/signalExplanationDescription — by population case-mix change, but this is deliberately modeled as a SEPARATE construct from rootCauseEstablished, which is now reserved exclusively for analytical causes. See case-validator.js\'s corrective-closure rule: rootCauseEstablished cannot be true when disturbanceEstablished is false.',
  ...
  'pilot-3-rcv-patient-impact': {
    ...
    rootCauseStatus: 'NOT an analytical root cause (rootCauseEstablished=false). The RCV exceedance is modeled as signalExplanationEstablished=true/signalExplanationDescription — a defensible STATISTICAL finding only (the change exceeds expected combined analytical + within-subject biological variation under the case\'s stated CVA/CVI assumptions). Corrected during the Stage 12A independent-audit closure: the case previously over-interpreted this as establishing "a genuine biological/clinical change," which RCV cannot determine (RCV exceedance ≠ disease, and does not establish etiological cause). A specimen-handling/preanalytical evidence item was added (ev-specimen-handling) so the case does not falsely eliminate every alternative through IQC/EQA alone — hyp-preanalytical-factor is WEAKENED, never CONTRADICTED, preserving genuine residual uncertainty.',
