...
`summarizeNpedTrials(trials)`: `anped` = mean NPed **only** when `allTrialsDetected=true`. When any trial is undetected, that trial's detection time is right-censored (known only to exceed the finite simulation horizon), so an ordinary arithmetic mean cannot incorporate it: `anped === undefined` internally — never averaged over the detected-only subset and presented as ANPed. The associated wording (`censoringNote`) states this as ANPed **"Not estimable"** (SC27 scientific owner adjudication, Dr Prasenjit Mitra + ChatGPT, superseding the prior "undefined" terminology) rather than a bare undefined/blank value, and explains that averaging only detected runs would bias the apparent detection delay downward. Separated `meanNpedAmongDetected` and `detectionRatePercent` are returned alongside when censoring occurs, so probability of detection and detection delay remain distinct questions. No artificial NPed value (horizon or horizon+1) is ever assigned to an undetected trial, and no survival-analysis functionality is introduced.

**Implementation status (v0.9.0):** `summarizeNpedTrials` and its matching UI component `MultiTrialNpedSummary` are implemented and independently tested against this invariant (see Tests below), but neither is imported by `app/pbrtqc/screens.jsx` — this invariant governs code that is not currently reachable from the shipped `#/pbrtqc` learner screen. The doctrine has been corrected prospectively, ahead of that component ever being wired in, so the corrected wording is already in place if/when it is exposed. See the v1.1+ backlog in `docs/RELEASE_BLOCKERS.md`.
