import { TARGET_VALUE_TYPES, describeSchemeCapability } from "./calc.js";
...
/* -------------------------------------------------------------------------
   ...
   (spec sections 4-5).
   ------------------------------------------------------------------------- */
export const EXTERNAL_ASSURANCE_PATHWAY_STEPS = ["Internal stability", "External comparison", "Understand the target", "Interpret deviation", "Assess method/laboratory effect", "Review longitudinally", "Investigate"];
export const EXTERNAL_ASSURANCE_PATHWAY_CAUTION = "This is an educational reasoning pathway, not a rigid universal sequence — real EQA interpretation may revisit earlier steps as evidence emerges, exactly as the Investigation Lab's own reasoning pathway does.";

export const GOOD_IQC_DOES_NOT_PROVE_TRUENESS_NOTE = "Good IQC does not prove trueness or inter-laboratory comparability.";
export const POOR_EQA_DOES_NOT_AUTOMATICALLY_PROVE_BIAS_NOTE = "A poor EQA result does not automatically prove that routine patient results are biased. Interpretation depends on the design and evidentiary capability of the EQA scheme.";

/* -------------------------------------------------------------------------
   Terminology (spec section 5).
   ------------------------------------------------------------------------- */
export const EQA_TERMINOLOGY = [
...
export const EQA_PT_TERMINOLOGY_CAUTION = "Terminology and regulatory usage vary between settings and countries, and PT is commonly used as one EQA approach among others — but not every EQA programme is identical to every regulatory PT scheme. This application uses \"EQA\" as the general umbrella term and \"PT\" where a scenario specifically concerns a proficiency-testing-style programme.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const CORE_THREE_WAY_DISTINCTION = [
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const EQA_NOT_REALTIME_IQC_NOTE = "EQA/PT is generally periodic rather than continuous and therefore does not substitute for surveillance of day-to-day analytical stability.";
export const EQA_CAN_REVEAL = [
...
export const EQA_REVEAL_DEPENDENCY_NOTE = "What a given EQA round can reveal depends on scheme design — not every scheme can reveal every item on this list. This application does not imply EQA can routinely detect today's analytical failure quickly enough to replace IQC.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const EQA_RESULT_FIELDS = [
...
export const MISSING_FIELDS_STAY_MISSING_NOTE = "Not every field in this structure has to be populated in every scenario. Where a scheme does not report something, this application leaves it missing rather than inventing a plausible-looking value.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const TARGET_VALUE_TYPE_DESCRIPTIONS = [
...
export const TARGET_TYPE_CLASSIFICATION_ITEMS = [
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const PEER_GROUP_NOT_TRUTH_PRINCIPLE = "Agreement with a peer group demonstrates agreement with that peer group; it does not necessarily demonstrate agreement with a higher-order reference.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const SCHEME_CAPABILITY_INPUTS = [
...
export const CAPABILITY_MILLER_ATTRIBUTION_NOTE = "This capability-based reasoning approach is informed by the framework Miller et al. (2011) described for characterising EQA/PT capability — see Evidence for the citation. This application implements an original capability interface built from the properties above rather than reproducing the published category table, and does not implement automated Category 1-6 classification.";

export const CAPABILITY_PARTICIPANT_PERFORMANCE_NOTE = "A scheme with a suitably defined peer group may provide evidence about whether this participant differs from comparable participants. This is not automatically evidence about absolute trueness.";
export const CAPABILITY_METHOD_PERFORMANCE_NOTE = "When appropriate materials, grouping and target assignment exist, EQA may also identify systematic differences affecting an entire measurement procedure or manufacturer group. This must be distinguished from a participant-specific laboratory problem.";
export const CAPABILITY_HARMONISATION_NOTE = "Assessment of between-method comparability is strongest when suitable commutable materials and higher-order target assignment are available. A noncommutable peer-group scheme cannot independently establish patient-sample harmonisation across methods.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const REPORT_INTERPRETATION_QUESTIONS = [
...
export const PEER_GROUP_HIDES_METHOD_BIAS_REPORT = {
...
export const LABORATORY_SPECIFIC_DEVIATION_REPORT = {
...
export const TARGET_CHANGES_CONCLUSION_REPORT = {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const SINGLE_EVENT_VS_TREND_NOTE = "One unusual EQA result deserves review but does not, by itself, establish persistent longitudinal bias. Conversely, repeated smaller deviations in the same direction may become important even when individual rounds remain within a broad acceptance criterion.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const LONGITUDINAL_EQA_TIMELINE = [
...
export const LONGITUDINAL_STATUSES_ARE_DESCRIPTIONS_NOTE = "These longitudinal classifications are scenario descriptions, not universal statistical diagnoses computed from a formula.";

/* -------------------------------------------------------------------------
   No automatic trend root cause (spec section 31).
   ------------------------------------------------------------------------- */
export const NO_AUTOMATIC_TREND_ROOT_CAUSE_NOTE = "Persistent EQA bias may be compatible with a calibration/traceability problem, a method-group bias, a laboratory implementation problem, a reagent/calibrator lot effect, or another systematic analytical difference. An EQA pattern alone does not identify which mechanism is responsible.";

/* -------------------------------------------------------------------------
   EQA + IQC combined matrix (spec sections 32-34).
   ------------------------------------------------------------------------- */
export const IQC_EQA_COMBINED_MATRIX = [
  {
...
export const STABLE_IQC_POOR_EQA_CAUTION = "Do not simply say \"calibration is wrong.\" The process may be precise/stable around a displaced value or may have a systematic alignment issue not visible to internal QC alone — several mechanisms remain possible.";
export const UNSTABLE_IQC_ACCEPTABLE_EQA_CAUTION = "A single satisfactory EQA event does not negate evidence of unstable routine analytical performance. EQA is a sparse periodic sample of performance.";

/* -------------------------------------------------------------------------
   EQA sample handling integrity (spec sections 35-36).
   ------------------------------------------------------------------------- */
export const SAMPLE_HANDLING_INTEGRITY_NOTE = "EQA/PT specimens should be processed in a manner representative of routine examination practice according to the applicable programme/laboratory requirements.";
export const SAMPLE_HANDLING_DISCOURAGED = [
...
export const PT_INTEGRITY_CHALLENGE = {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const EQA_PROCESS_ERROR_TYPES = [
...
export const INVESTIGATE_WHOLE_EQA_PROCESS_NOTE = "The learner should investigate the whole EQA process, not assume analyser bias by default.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const COMPARABILITY_LAB_SCOPE_NOTE = "This is not a full CLSI EP09 method-comparison module. Calculations here are deliberately simple and transparent: paired differences and relative differences only.";
export const DESIGNATED_COMPARATOR_NOTE = "Analyzer A is called the designated comparator for this teaching exercise, never \"the reference method.\" This application does not automatically treat either analyser as truth.";
export const EXCLUDED_METHOD_COMPARISON_STATISTICS = [
...
export const EXCLUDED_STATISTICS_NOTE = "These may become a dedicated advanced comparability module later if desired. They are explicitly excluded from v0.6.";
...
export const COMPARABILITY_PAIRED_SPECIMENS = [
...
export const COMPARABILITY_LIMIT_LABEL = "Scenario-specific comparability criterion";
export const COMPARABILITY_LIMIT_CAUTION = "Where a teaching scenario uses a numeric criterion, it is labelled a scenario-specific comparability criterion, and its source (or illustrative status) is stated explicitly. This application does not invent a universal allowable between-analyser difference.";
export const COMPARABILITY_LAB_ILLUSTRATIVE_CRITERION = { value: "±5%", source: "Illustrative for this teaching exercise only — not drawn from any regulatory or manufacturer document." };

/* Control-material comparability trap (spec section 43). */
export const CONTROL_MATERIAL_TRAP_CASE = {
...
export const PATIENT_COMPARISON_TRAP_CASE = {
...
export const LONGITUDINAL_COMPARABILITY_SUMMARY = [
...
export const NOT_EVERY_DIFFERENCE_CLINICALLY_IMPORTANT_NOTE = "Do not label every difference clinically important. Whether a given between-analyser difference matters clinically depends on the analyte, the magnitude relative to a stated comparability criterion, and the clinical use of the result.";

/* -------------------------------------------------------------------------
   EQA investigation path (spec section 55) — a reasoning framework, not a
   universal mandated sequence.
   ------------------------------------------------------------------------- */
export const EQA_INVESTIGATION_PATH_STEPS = [
...
export const EQA_INVESTIGATION_PATH_CAUTION = "This is a reasoning framework, not a universal mandated sequence — as with the Investigation Lab's own pathway, a real EQA investigation may revisit earlier steps as evidence emerges.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const EXPLORE_INVESTIGATION_LINK_LABEL = "Explore how to investigate this";
export const NO_AUTO_TRANSFER_BETWEEN_MODULES_NOTE = "Where an EQA result warrants investigation, this application links to the existing Investigation Lab reasoning framework rather than duplicating its engine, and does not automatically transfer scenario truth states between modules.";

/* -------------------------------------------------------------------------
   Mandatory guardrail statements (spec sections 50-53).
   ------------------------------------------------------------------------- */
export const NO_EQA_PASS_EQUALS_METHOD_VALID_NOTE = "A satisfactory result in one EQA event provides useful external evidence but does not by itself validate every aspect of the measurement procedure.";
export const NO_EQA_FAIL_EQUALS_PATIENT_RESULTS_WRONG_NOTE = "An unacceptable or unexpected EQA result requires investigation; its implications for routine patient results depend on the nature of the error, material commutability, timing, method behaviour and corroborating evidence. This application does not automatically reopen historical patient results from one EQA event.";
export const NO_PEER_MEAN_EQUALS_REFERENCE_VALUE_NOTE = "A peer-group mean is a consensus comparator for that group, not automatically a metrologically higher-order reference.";
export const NO_COMMUTABLE_EQUALS_PERFECT_NOTE = "Verified commutability supports interpretation of relationships among measurement procedures for relevant clinical samples; it does not guarantee that the target assignment, performance criterion or all other programme features are optimal.";

/* -------------------------------------------------------------------------
   No auto-patient-impact from EQA (spec sections 75-76).
   ------------------------------------------------------------------------- */
export const NO_AUTO_PATIENT_IMPACT_FROM_EQA_NOTE = "An unacceptable EQA event must never automatically set historical patient results to held, invalid, amend, or reissue in this application. Where patient impact needs assessment, this routes conceptually to the validated Investigation Lab reasoning framework rather than duplicating its engine here.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const PATTERN_JUDGEMENT_OPTIONS = [
...
export const CAPABILITY_CONCLUSION_OPTIONS = [
...
export const LONGITUDINAL_RELEVANCE_OPTIONS = [
...
export const EXTERNAL_ASSURANCE_STAGES = ["target", "commutability", "capability", "pattern", "longitudinal", "next-step", "confidence"];

export const EXTERNAL_ASSURANCE_CASES = [
  {
