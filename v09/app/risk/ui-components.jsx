...
import { fmt } from "../core/statistics.js";
import { operatingCharacteristic13s } from "../opchar/functions.js";
import { M_LABEL, N_LABEL, N_R_M_DISTINCTION_NOTE, QUADRANT_MATRIX, QUADRANT_MATRIX_NOTE, R_LABEL, SHIFT_CAUSE_CAUTION, SYNTHETIC_SHIFT_INTRODUCED_TEXT } from "./data.js";
import { UNSUPPORTED_DETECTION_DELAY_NOTE, expectedQcEventsToDetectionGeometric, isDetectionDelaySupportedRuleSet } from "./detection-delay.js";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function DetectionDelayResultCard({ title, result }) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function NRMPanel({ N, R, M }) {
  return (
    <div>
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function QuadrantMatrix({ highlightId }) {
  return (
    <div>
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const TIMELINE_INTERVALS = 4; // fixed regardless of M — only the interval's internal scale changes

export function intervalWidthPx(M) {
  const w = 46 + Math.log2(Math.max(M, 1) + 1) * 22;
  return Math.max(60, Math.min(220, Math.round(w)));
}
export function iconCountFor(M) {
  return Math.max(3, Math.min(8, Math.round(Math.log2(Math.max(M, 1) + 1))));
}

export function QCTimeline({ M, shiftSD, N, onsetMode, onsetPositionPct, ruleIds }) {
