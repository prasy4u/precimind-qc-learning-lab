import { PARAMETER_PROVENANCE_OPTIONS, VERIFICATION_LEAKAGE_NOTE } from "./data.js";
import { MetricCard } from "../ui/shared-components.jsx";
...
export function humanizePbrtqcLabel(s) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const PBRTQC_ALERT_ICONS = { alert: "▲", "no-alert": "●", "warming-up": "…", excluded: "✕" };
export const PBRTQC_INCLUSION_ICONS = { included: "●", excluded: "✕" };
export const PBRTQC_DETECTION_ICONS = { detected: "✓", undetected: "?" };

export function PbrtqcStatusBadge({ kind, value, labelOverride }) {
...
/* -------------------------------------------------------------------------
   ...
   available.
   ------------------------------------------------------------------------- */
export const MAX_RENDERED_POINTS = 180;

export function samplePoints(points) {
...
export function PatientStreamChart({ points, errorOnsetRawIndex, lowerControlLimit, upperControlLimit }) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function MovingStatisticSummary({ result }) {
...
export function EwmaSummary({ result }) {
  if (!result || !result.supported) return <p className="muted small">{result ? result.reason : "No result."}</p>;
  const series = result.series;
  const last = series[series.length - 1];
  return (
...
export function MultiTrialNpedSummary({ summary }) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function TrainingVerificationDisclosure({ trainingLabel, verificationLabel }) {
  return (
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function ParameterProvenanceNote({ provenanceId, paramLabel }) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function ChangedVsConstantPanel({ changed, heldConstant }) {
  return (
    <div className="quadrant-grid">
      <div className="quadrant-cell">
        <h4>Changed</h4>
