...
### Pre-Release Visual Polish

A bounded, audit-adjudicated visual-polish closure before first public release. **No scientific, engine, case, analytics, research, storage, scoring or competency file was touched** — verified by targeted `git diff` guard. All four source changes are presentation-only.

**P0 (mandatory).** Fixed the `.calc-panel` monospace leakage: the class forced `var(--font-mono)` unconditionally, so multi-sentence QC-03 teaching prose rendered in monospace (three sites already carried inline `fontFamily: inherit` workarounds; the handling-doctrine block had none). Split into `.calc-panel` (numeric/formula, retains mono) and `.calc-panel-prose` (body font), removing the inline hacks. Added a global 44px interactive-target minimum via `--target-min`, excluding non-interactive badges. Extended the token system additively with `--radius-sm/md/lg` and `--shadow-sm/md` (existing `--radius`/`--shadow` untouched so all current references keep working) and applied three surface tiers. Added a `--space-1..8` scale applied to principal shared components. Darkened `--text-faint` `#8894a0 → #6e7a86` (~3.0:1 → ~4.6:1) in light and lightened `#7c8994 → #8d99a4` in dark, updated across all three theme blocks.

**P1.** Typographic hierarchy strengthened with mobile step-downs. Navigation presentation refined (tighter padding, product/nav divider, three-channel active state) — 14 destinations, order and semantics unchanged. Semantic panel differentiation for prompt/info/warning/result/basis roles. QC-03 guided progression: `done/current/upcoming` classes derived *purely* from the existing station index with a "Station N of 6" label — no persistence, no global-progress write, no competency state, tab semantics and free selection preserved; comparison cards relaid out as a genuine two-column comparison with content unchanged. Morning QC re-weighted — rails moved to `--surface-2` (they were previously the *brightest* surface, competing with the workspace), workspace elevated, Patient Impact given a neutral accent treatment that deliberately avoids warning/danger tints, action bar reconnected. Chart reference lines now differentiated by dash pattern *and* weight rather than colour alone; chart maths, geometry and scaling untouched.

**QA.** Real-Chromium capture across light and dark themes at 1920×1080 and 390×844: **0 page exceptions, 0 console errors, 0 horizontal overflow**. Regression: engine 49/49, case bank 221/221, numeric audit 28/28, QC-03 53/53, UI 105/105, Stage 12A 121/121, 12B 65/65, 12C 48/48, 12D 48/48, 12E 26/26, 12F 89/89, Stage 11B 67/67, 11C2 49/49, integration 30/30; historical evidence 27/27 byte-identical. Scope allowlists in Stage 12A/12C/12D were extended to cover the authorized VISUAL-TOKENS / GLOBAL-CSS / MORNING-QC-PRESENTATION change classes.

Production hash advanced `587a0917f9… → d3caa1532fd7635f533eebbbf5cd9cf51ef99d5aa011043b320a505c6246c958`, reproducible across two consecutive rebuilds.

### Visual Polish — Final Accessibility Micro-Correction

Closes the two accessibility findings from independent audit of the visual-polish pass. **Zero scientific/behavioural change** — the freeze guard diff over engine, cases, states, decision/scoring models, analytics, research, adaptive, `core/`, QC-03 data and `src/` was confirmed empty; only CSS, QA evidence and build/provenance hashes changed.

**Dark-mode case-title contrast.** `.mqc-case-select__card` is a `<button>` with a themed background but declared no foreground colour, so Morning QC case titles fell back to the UA default button colour (near-black) and were effectively illegible on the dark card surface. Added `color: var(--text)` — theme token, not hardcoded white — so the title inherits correctly while the focus/difficulty/status children keep their own muted and semantic colours. Measured contrast in live Chromium improved to **14.24:1**; supporting text verified as still visually subordinate with no unintended inheritance. The recommended-case card reuses the same class and is covered by the same fix.

**Completed the 44px target requirement.** `.btn-icon` (32×32), `.case-chip` (38px min-width), `.btn-tiny` (~22px) and `.seg` (~34px) all remained below the minimum after the first pass. Each now uses the existing `--target-min` token rather than a new hardcoded value, with compact visual character preserved through unchanged font-size and horizontal padding — only the hit box grows, via flex centring. Badges, non-interactive chips and labels were deliberately not inflated.
