import { runPbrtqcStream } from "./calc.js";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const PATIENT_POPULATION_SCENARIO_FIELDS = [
...
export const ANALYTICAL_ERROR_SCENARIO_FIELDS = [
...
export const PBRTQC_ALGORITHM_CONFIGURATION_FIELDS = [
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const N_R_M_W_DISTINCTION_EXAMPLE = {
...
export const ALGORITHMS_MENTIONED_NOT_IMPLEMENTED = [
...
export const ALGORITHMS_EXCLUDED_FROM_V08 = [
  "CUSUM-based PBRTQC.",
...
export const ALGORITHM_SCOPE_NOTE = "This lab implements exactly three algorithms numerically: sliding moving mean, sliding moving median, and EWMA — all evaluated as SLIDING statistics (recomputed at every new eligible result), never as non-overlapping blocks. Other approaches described in the literature are mentioned for context but not computed anywhere in this build.";

/* -------------------------------------------------------------------------
   Sliding vs. non-overlapping blocks (spec sections 28-29).
   ------------------------------------------------------------------------- */
export const SLIDING_VS_BLOCKS_NOTE = "A SLIDING statistic recomputes at every new eligible result, using the W most recent eligible values each time (overlapping windows). A NON-OVERLAPPING BLOCK approach instead computes one statistic per separate block of W results, with no overlap between blocks. This application's numeric engine uses sliding statistics for both the moving mean and the moving median — it never implements non-overlapping blocks.";
...
export const METADATA_FILTER_CAUTION = "No metadata exclusion rule listed here is universally appropriate for every laboratory or every analyte. Each is a synthetic teaching example, illustrating a trade-off (representativeness vs. homogeneity of the monitored stream), not a recommended default.";

/* -------------------------------------------------------------------------
   Truncation (spec sections 32-37).
   ------------------------------------------------------------------------- */
export const TRUNCATION_NOTE = "This application implements exactly ONE truncation approach: hard exclusion. A result strictly below the lower truncation limit, or strictly above the upper truncation limit, is excluded from the algorithm entirely. A result exactly AT a limit remains eligible. This is never called \"winsorisation\" — winsorisation instead clamps an out-of-range value to the limit rather than excluding it, and is a different technique not implemented anywhere in this build.";
export const NO_UNIVERSAL_TRUNCATION_NOTE = "This application never states a \"recommended universal truncation limit\" for any real analyte. Every truncation limit shown is a scenario-specific, illustrative teaching value.";

/* -------------------------------------------------------------------------
   Parameter provenance (spec section 88) — every configuration must state
   how its parameters were obtained.
   ------------------------------------------------------------------------- */
export const PARAMETER_PROVENANCE_OPTIONS = [
...
export const STARTING_CONFIGURATION_LANGUAGE_NOTE = "This application always calls an initial parameter set a \"starting teaching configuration,\" never a \"recommended configuration\" — unexplained defaults are never implied to be optimal.";

/* -------------------------------------------------------------------------
   Control limits (spec sections 50-53).
   ------------------------------------------------------------------------- */
export const CONTROL_LIMIT_PROVENANCE_NOTE = "This application never hard-codes a universal \"mean +/- 2SD\" or \"mean +/- 3SD\" control limit for PBRTQC. Every scenario displays HOW its limits were obtained, using the same PARAMETER_PROVENANCE_OPTIONS categories used for every other configuration choice.";
export const ALERT_BOUNDARY_NOTE = "Alert boundary convention (v0.8): a statistic strictly below the lower control limit, or strictly above the upper control limit, triggers an alert. A statistic exactly equal to a control limit does NOT trigger an alert.";
export const CONTROL_LIMIT_TRADEOFF_NOTE = "Narrower control limits detect smaller shifts sooner but increase the false-alert rate. Wider control limits reduce false alerts but delay or miss smaller shifts. There is no universally optimal trade-off — the right balance depends on the analyte, the population, and the clinical consequences of a missed or a false alert.";

/* -------------------------------------------------------------------------
   Mandatory persistent statements (spec sections 80-84).
   ------------------------------------------------------------------------- */
export const ALERT_INTERPRETATION_STATEMENT = "A PBRTQC alert indicates that the configured patient-based statistic crossed its control limit. It does not identify why.";
export const STABLE_PBRTQC_DOES_NOT_VALIDATE_STATEMENT = "A stable PBRTQC statistic does not prove that every individual patient result is analytically correct.";
export const PBRTQC_NOT_DELTA_CHECK_STATEMENT = "PBRTQC is not a delta check. A delta check compares two serial results from the SAME patient (see the BV & RCV Lab's reference change value tools); PBRTQC evaluates a moving statistic across an aggregate stream of many different patients.";
export const PBRTQC_NOT_EQA_STATEMENT = "PBRTQC is not EQA. External quality assessment (see the External Assurance Lab) compares a laboratory's result on a shared, characterised material against peer or reference values at a scheduled interval; PBRTQC continuously monitors routine patient results between those events.";
export const ALERT_ROUTES_TO_INVESTIGATION_NOTE = "Interpreting what a PBRTQC alert might mean is routed conceptually to the existing Investigation Lab via an \"Investigate this alert\" link. This application never automatically transfers a PBRTQC alert into a confirmed process-failure state — the two modules do not share mutable scenario state.";

/* -------------------------------------------------------------------------
   False-positive flag rate (spec sections 44-46) and training/verification
   architecture (spec sections 47-49, 90-95).
   ------------------------------------------------------------------------- */
export const FALSE_FLAG_RATE_NOTE = "A formal false-positive flag rate is calculated ONLY on a verification stream explicitly designated stable, with no injected error and no case-mix shift: pointwise false-flag rate = (points outside the control limits / evaluable points) x 100. Consecutive moving-window statistics are correlated with one another (they share overlapping eligible values), so this is not equivalent to a set of independent hypothesis tests and is never called simply \"alpha.\"";
export const TRAINING_VERIFICATION_SEPARATION_NOTE = "Training and verification datasets are architecturally separate fields in this application, never one shared field reused for two purposes. The TRAINING dataset supports familiarisation, parameter selection, and candidate control limits. The VERIFICATION dataset supports baseline false-alert assessment, error insertion, and detection-performance verification.";
export const VERIFICATION_LEAKAGE_NOTE = "Repeatedly tuning a configuration against the verification dataset, then reporting the resulting performance figures as if they came from an independent verification, is NOT independent verification. This is a form of data leakage, and this application teaches it as a signature mistake to recognise and avoid.";
export const HISTORICAL_DATA_NOTE = "Developing a PBRTQC configuration requires enough historical data to characterise the population and estimate baseline behaviour reliably — there is no single universal minimum number of results that applies to every analyte and laboratory. Training/verification split examples such as 50:50 or 80:20 (see Loh et al., 2020) are shown as EXAMPLES from the literature, not as a mandated split.";

/* -------------------------------------------------------------------------
   No live data (spec sections 96-99).
   ------------------------------------------------------------------------- */
export const NO_LIVE_DATA_NOTE = "This module uses no live patient data of any kind: no CSV upload, no LIS/analyser interface, no identifiable data, and no real historical datasets. Every population in the Patient Distribution Lab is a small, deterministic, pre-generated synthetic array, always labelled accordingly.";
export const SYNTHETIC_DATA_CARD_LABEL = "Synthetic deterministic teaching data. Not derived from real patients.";
export const PRIVACY_BRIEF_NOTE = "A real PBRTQC implementation processes identifiable patient data and must be governed by the laboratory's data-protection and privacy obligations. This application does not build any privacy infrastructure — it never collects, stores, or transmits real patient data, because none is ever used.";

/* -------------------------------------------------------------------------
   Informatics reliability (spec sections 100-102) — taught conceptually.
   ------------------------------------------------------------------------- */
export const INFORMATICS_RELIABILITY_TOPICS = [
...
export const INFORMATICS_CHALLENGE_SCENARIO = {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const VALIDATION_LIFECYCLE_STEPS = ["Training (familiarisation)", "Parameter selection", "Independent verification", "Implementation", "Ongoing review", "Re-verification after material change"];
export const VALIDATION_LIFECYCLE_NOTE = "This lifecycle never implies a PBRTQC configuration is \"validated once, forever.\" Ongoing review and re-verification after a material change are part of the same lifecycle, not an optional afterthought.";
export const MATERIAL_CHANGE_EXAMPLES = [
...
export const MATERIAL_CHANGE_LIST_CAUTION = "This is not an exhaustive or universal trigger list — what counts as a \"material change\" requiring re-verification is a laboratory-specific judgement.";
export const NO_AUTO_OPTIMIZER_NOTE = "This application deliberately does NOT implement a \"find the best PBRTQC configuration\" auto-optimiser. Parameter selection is shown as a reasoned, documented choice with stated provenance, never as a button that searches for and returns an optimal answer.";
export const NO_UNIVERSAL_TARGETS_NOTE = "This application never states a universal performance target (for example, \"ANPed < 20\" or \"false-flag rate < 1%\" or \"W = 20 is the correct window size\" or \"lambda = 0.2 is the correct smoothing constant\") as a fixed requirement for every laboratory and analyte. Every numeric target shown is scenario-specific and illustrative.";
export const IQC_FREQUENCY_CONNECTION_EXPERIMENT = {
...
export const V04_ANPED_REMINDER_NOTE = "v0.4 introduced ANPed conceptually within the Risk & Frequency Lab's patient-risk framework. This module reuses the SAME NAME for a different but related quantity (mean patient results before detection, here computed across PBRTQC error-injection trials) and does not rewrite or reconnect to v0.4's patient-risk engine — see the distinct-metrics note below.";
export const NPED_VS_MAXENUF_NOTE = "NPed/ANPed and MaxE(Nuf) are different metrics derived in different quality-control frameworks. This application never combines their formulas or presents one as a substitute for the other.";
export const REPORT_FROM_BACK_NOTE = "\"Report from the back\" — holding a block of results pending completion of the block before releasing them — is mentioned here only as a conceptual approach some laboratories use alongside PBRTQC. This application does not implement any automated result-holding or auto-release logic.";

/* -------------------------------------------------------------------------
   Throughput (spec sections 74-75).
   ------------------------------------------------------------------------- */
export const THROUGHPUT_NOTE = "Patient-count detection delay (NPed, measured in patient results) is not the same as elapsed-time detection delay. The same NPed=50 means roughly 1 hour at a throughput of 50 results/hour, but roughly 10 hours at a throughput of 5 results/hour.";
export const THROUGHPUT_EXAMPLE = { nped: 50, labA: { resultsPerHour: 50, elapsedHours: 1 }, labB: { resultsPerHour: 5, elapsedHours: 10 } };
export const THROUGHPUT_ASSUMPTION_LABEL = "Simplified constant-throughput teaching assumption — real submission rates vary by time of day, day of week, and clinical demand, so this elapsed-time figure is illustrative, not a prediction.";

/* -------------------------------------------------------------------------
   Multiple analyzers (spec sections 76-77).
   ------------------------------------------------------------------------- */
export const MULTIPLE_ANALYZER_NOTE = "Combining patient results across instruments changes the monitored process and must be validated for that configuration.";
export const MULTIPLE_ANALYZER_EXPERIMENT = {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const ERROR_DETECTION_RECOMMENDED_FLOW = ["Population", "Inclusion / truncation", "Error injection", "Algorithm", "Control limits", "Alert", "NPed"];

/* -------------------------------------------------------------------------
   Workflow questions A-G + confidence (spec sections 125-127).
   ------------------------------------------------------------------------- */
export const PBRTQC_WORKFLOW_QUESTIONS = [
...
export const PBRTQC_CONFIDENCE_NEVER_ALTERS_SCORE_NOTE = "Confidence is recorded for your own metacognitive reflection only. It never changes the scientific-reasoning score for a case.";
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const FORBIDDEN_BLANKET_STATEMENTS = [
  "PBRTQC replaces IQC.",
...
export const METHODOLOGICAL_DEVELOPMENT_NOTE = "PBRTQC is an active area of methodological development. Algorithm and parameter choices require local validation rather than uncritical transfer from another laboratory.";

/* -------------------------------------------------------------------------
   Exclusion list (spec section 169).
   ------------------------------------------------------------------------- */
export const PBRTQC_EXCLUSION_LIST = [
...
/* =========================================================================
   ...
   provenance line.
   ========================================================================= */
export const POPULATION_A_VALUES = [
...
export const POPULATION_B_VALUES = [
...
export const POPULATION_C_VALUES = [
...
export const POPULATION_D_VALUES = [
...
export const POPULATION_D_SUBGROUPS = [
...
export const POPULATION_E_VALUES = [
...
/* -------------------------------------------------------------------------
   ...
   test-pbrtqc.js.
   ------------------------------------------------------------------------- */
export const DISTRIBUTION_SIGNATURE_EXPERIMENT = {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const POPULATION_SHIFT_WITHOUT_ERROR_SCENARIO = {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const AGGRESSIVE_TRUNCATION_EXPERIMENT = {
...
/* =========================================================================
   ...
   ========================================================================= */
export const PBRTQC_CHALLENGE_CASES = [
  {
...
export const PBRTQC_ANSWER_KIND_OPTIONS = {
