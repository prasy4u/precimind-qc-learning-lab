...
import { RULE_LABELS } from "../rules/engine.js";
import { frameworkWhyText, sequentialObservationCapacity } from "./core.js";
import { Badge } from "../ui/shared-components.jsx";
import { OPCHAR_SCOPE_NOTE } from "./core.js";
...
export function ruleLabelList(ruleIds) {
...
/* Ordinal, clearly-labelled-as-qualitative complexity/detection/burden
   ...
   results are. */
export function qualitativeComplexity(proc) {
  const score = proc.ruleIds.length + sequentialObservationCapacity(proc) / 2;
  if (score <= 3) return "Low";
  if (score <= 5) return "Moderate";
  return "High";
}
export function qualitativeDetection(proc) {
...
export function qualitativeFalseRejectionBurden(proc) {
...
export function FrameworkProvenanceNote({ mapping }) {
...
export function ProcedureCard({ procedure, highlight }) {
  return (
...
export function ComparatorTable({ procedures, deltaSE }) {
  return (
