...
export function exceedsPositive(z, limit) { return typeof z === "number" && isFinite(z) && z > limit; }
export function exceedsNegative(z, limit) { return typeof z === "number" && isFinite(z) && z < -limit; }
export function exceedsAbs(z, limit) { return exceedsPositive(z, limit) || exceedsNegative(z, limit); }
export function sideOf(z) { return z > 0 ? "positive" : (z < 0 ? "negative" : "zero"); }

export function flattenPoints(runs) {
  const pts = [];
  runs.forEach(run => {
...
export function pointsByLevel(runs, levelId) {
  const pts = [];
  runs.forEach(run => {
...
export function levelIdsIn(runs) {
  const set = [];
  runs.forEach(run => (run.controlResults || []).forEach(cr => { if (set.indexOf(cr.levelId) === -1) set.push(cr.levelId); }));
  return set;
}

export function makeEvent(ruleId, status, triggerPoints, scope, direction, educationalInterpretation) {
...
/* 1_2s: WARNING only */
export function detect12s(runs) {
  const events = [];
...
/* 1_3s: REJECTION */
export function detect13s(runs) {
  const events = [];
  flattenPoints(runs).forEach(pt => {
...
export function detect22s(runs) {
  const events = [];
  runs.forEach(run => {
...
export function detectR4s(runs) {
  const events = [];
  runs.forEach(run => {
    const results = run.controlResults || [];
    for (let i = 0; i < results.length; i++) {
...
/* 4_1s: REJECTION */
export function detect41s(runs) {
  const events = [];
  levelIdsIn(runs).forEach(levelId => {
    const pts = pointsByLevel(runs, levelId);
...
export function detect10x(runs) {
  const events = [];
  levelIdsIn(runs).forEach(levelId => {
    const pts = pointsByLevel(runs, levelId);
...
export const RULE_ORDER = ["12s", "13s", "22s", "r4s", "41s", "10x"];

export function evaluateRuleSet(runs, enabledRuleIds) {
