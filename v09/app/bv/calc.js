...
export const Z_CONVENTION_IDS = Object.keys(Z_CONVENTIONS);

export function isFiniteNonNegativeNumber(x) {
...
/* -------------------------------------------------------------------------
   ...
   II > 1.4 -> low.
   ------------------------------------------------------------------------- */
export function calculateIndexOfIndividuality(cvi, cvg) {
...
export function calculateBvAps(cvi, cvg) {
  if (typeof cvi !== "number" || !isFinite(cvi) || cvi < 0) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function calculateClassicalRcv(cva, cvi, zConventionId) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function calculateLognormalRcv(cva, cvi, zConventionId) {
  if (!isFiniteNonNegativeNumber(cva) || !isFiniteNonNegativeNumber(cvi)) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export function calculateSerialAbsoluteChange(previousResult, currentResult) {
...
export function calculateSerialRelativeChange(previousResult, currentResult) {
  if (typeof previousResult !== "number" || typeof currentResult !== "number" || !isFinite(previousResult) || !isFinite(currentResult)) {
    return { supported: false, value: null, reason: "Both the previous and current results must be finite numbers." };
  }
  if (previousResult === 0) {
...
/* -------------------------------------------------------------------------
   ...
   never exceeds" rule.
   ------------------------------------------------------------------------- */
export const EXCEEDANCE_EPSILON = 1e-9;

export function directionOf(relativeChange) {
  if (relativeChange > 0) return "increase";
  if (relativeChange < 0) return "decrease";
  return "no-change";
}

export function evaluateClassicalRcvExceedance(previousResult, currentResult, cva, cvi, zConventionId) {
...
export function evaluateLognormalRcvExceedance(previousResult, currentResult, cva, cvi, zConventionId) {
  const change = calculateSerialRelativeChange(previousResult, currentResult);
  if (!change.supported) {
    return { supported: false, reason: change.reason };
  }
