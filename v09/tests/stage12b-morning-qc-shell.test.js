...
async function main() {
  ...
  console.log('\n=== 2. Stage 12A engine files byte-frozen (with documented sanctioned exceptions) ===');
  {
    ...
    assert('2b', exceptions && Array.isArray(exceptions.files) && exceptions.files.length === 7, 'Manifest explicitly documents exactly 7 sanctioned exceptions from the original Stage 12A baseline (case-schema.js + all 3 pilot cases + scoring-model.js + case-validator.js + engine.js)');
    assert('2c', exceptions.files.includes('app/morning-qc/case-schema.js') && exceptions.files.includes('app/morning-qc/cases/pilot-1-reagent-lot-shift.js') && exceptions.files.includes('app/morning-qc/cases/pilot-2-pbrtqc-population-shift.js') && exceptions.files.includes('app/morning-qc/cases/pilot-3-rcv-patient-impact.js') && exceptions.files.includes('app/morning-qc/scoring-model.js') && exceptions.files.includes('app/morning-qc/case-validator.js') && exceptions.files.includes('app/morning-qc/engine.js'), 'The 7 documented exceptions are exactly case-schema.js, all 3 pilot cases, scoring-model.js, case-validator.js, and engine.js (never states.js, decision-model.js, or debrief-model.js)');
