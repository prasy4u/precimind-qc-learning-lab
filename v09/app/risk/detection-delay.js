...
export const UNSUPPORTED_DETECTION_DELAY_NOTE = "Numerical detection-delay modelling is not implemented for this multirule procedure in the current version.";
...
export function isDetectionDelaySupportedRuleSet(ruleIds) {
  return Array.isArray(ruleIds) && ruleIds.length === 1 && ruleIds[0] === "13s";
}
export function detectionDelaySupportForRuleIds(ruleIds) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const GEOMETRIC_MODEL_ID = "geometric-persistent-shift-detection-delay";
export const GEOMETRIC_MODEL_NAME = "Simplified persistent-shift detection model (geometric)";
export const GEOMETRIC_ASSUMPTIONS = [
...
export function expectedQcEventsToDetectionGeometric(p) {
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
export const IMMEDIATE_ONSET_MODEL_ID = "geometric-immediate-onset-patient-exposure";
export const IMMEDIATE_ONSET_MODEL_NAME = "Simplified persistent-shift detection model — immediate post-QC onset (Mode A)";
export const IMMEDIATE_ONSET_ASSUMPTIONS = GEOMETRIC_ASSUMPTIONS.concat([
...
export const IMMEDIATE_ONSET_LIMITATIONS = GEOMETRIC_LIMITATIONS.concat([
...
export function expectedPatientExposureImmediateOnset(M, p) {
...
export const UNIFORM_ONSET_MODEL_ID = "geometric-uniform-onset-patient-exposure";
export const UNIFORM_ONSET_MODEL_NAME = "Simplified persistent-shift detection model — uniformly distributed onset within interval (Mode B)";
export const UNIFORM_ONSET_ASSUMPTIONS = GEOMETRIC_ASSUMPTIONS.concat([
...
export const UNIFORM_ONSET_LIMITATIONS = IMMEDIATE_ONSET_LIMITATIONS;
...
export function expectedPatientExposureUniformOnset(M, p) {
  const invalidP = typeof p !== "number" || !isFinite(p) || p <= 0 || p > 1;
  const invalidM = typeof M !== "number" || !isFinite(M) || M <= 0;
  if (invalidP || invalidM) {
    return {
