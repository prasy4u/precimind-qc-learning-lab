...
export const pilot2PbrtqcPopulationShift = {
  ...
  evidence: [
    { id: 'ev-iqc-stable', source: 'panel-qc-history', timestamp: 780, observedValueOrFinding: 'All IQC runs remain within control throughout the PBRTQC trend window.', interpretationLimits: 'IQC passing does not, by itself, prove no analytical issue exists — but sustained normal IQC across the whole window is meaningful, non-decisive evidence against a general analytical shift.', supportsHypothesisIds: ['hyp-population'], weakensHypothesisIds: ['hyp-analytical'], decisive: false, relevant: true, availableOnlyAfterActionType: null },
    { id: 'ev-ward-timing', source: 'panel-patient-distribution', timestamp: 480, observedValueOrFinding: 'New cardiology step-down ward begins sending specimens at t=480, ~2 hours before the PBRTQC trend begins.', interpretationLimits: 'Temporal association alone does not establish causation.', supportsHypothesisIds: ['hyp-population'], weakensHypothesisIds: [], decisive: false, relevant: true, availableOnlyAfterActionType: null },
    { id: 'ev-case-mix-decisive', source: 'stratified re-analysis of PBRTQC statistic by ward', timestamp: 780, observedValueOrFinding: 'Recomputing the PBRTQC moving-mean statistic EXCLUDING the new ward\'s specimens shows the statistic remains stable and does not cross the alert threshold.', interpretationLimits: 'This isolates the case-mix change as the driver of the alert, since removing only the new population removes the signal entirely.', supportsHypothesisIds: ['hyp-population'], weakensHypothesisIds: ['hyp-analytical', 'hyp-calibration'], decisive: true, relevant: true, availableOnlyAfterActionType: 'CHECK_PATIENT_DISTRIBUTION' },
  ],
  groundTruth: {
