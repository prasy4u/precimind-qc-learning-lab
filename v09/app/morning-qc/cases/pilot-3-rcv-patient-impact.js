...
export const pilot3RcvPatientImpact = {
  ...
  evidence: [
    { id: 'ev-iqc-clean', source: 'panel-qc-history', timestamp: 480, observedValueOrFinding: 'IQC within control across the entire window.', interpretationLimits: 'Stable IQC does not prove trueness for this specific specimen, but it is evidence against a general, laboratory-wide analytical shift.', supportsHypothesisIds: ['hyp-statistically-significant-change'], weakensHypothesisIds: ['hyp-analytical-error'], decisive: false, relevant: true, availableOnlyAfterActionType: null },
  ...
  decisionOpportunities: [
    { id: 'dec-interpretation', category: 'INTERPRETATION', availableFromPhase: 'HYPOTHESIS_GENERATION', options: [
      { id: 'opt-apply-rcv', label: 'Apply RCV to assess statistical significance, without over-claiming biological certainty', consequenceSummary: 'Correct method for this question.', severity: 'INFORMATIONAL', outcomeAppropriate: true },
      { id: 'opt-assume-error', label: 'Assume an analytical error without checking IQC/EQA/RCV', consequenceSummary: 'Applies the forbidden inference for this case family.', severity: 'UNSUPPORTED', outcomeAppropriate: false },
    ]},
    { id: 'dec-disposition', category: 'DISPOSITION', availableFromPhase: 'RESUME_OR_HOLD', options: [
      { id: 'opt-no-hold-document', label: 'No analytical hold; document the RCV-based statistical finding, with correct interpretation limits, for the clinical team', consequenceSummary: 'Matches ground truth.', severity: 'INFORMATIONAL', outcomeAppropriate: true },
      { id: 'opt-hold-analytical', label: 'Hold analytical service pending an unnecessary analytical investigation', consequenceSummary: 'IQC and EQA both support ongoing validity; an analytical hold is not evidence-supported here.', severity: 'UNSUPPORTED', outcomeAppropriate: false },
      { id: 'opt-overclaim-biological', label: 'Document that the change definitively reflects a genuine biological/clinical cause', consequenceSummary: 'Over-interprets RCV exceedance as proof of biological etiology — RCV is a statistical threshold, not a diagnostic or causal determination.', severity: 'UNSUPPORTED', outcomeAppropriate: false },
    ]},
  ],
